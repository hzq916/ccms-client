angular.module('cmsController').controller('positionDataSyncHistoryCtrl', function ($scope, $timeout, mainService, positionDataSyncHIstoryService) {
	...
	//仓位数据同步同步记录中账户类型和同步状态
	$scope.currentAccountType = {};

	$scope.positionDataAccountType = [
		{
			id: "0",
			name: "请选择账户类型"
		},
	...
	$scope.positionDataSyncDetail = [];



	$scope.positionDataSyncHistoryInit = function () {
		$scope.searchContent = "";
		$scope.modalInfo.state = 0;                      //默认值，无定义
		$scope.modalInfo.stateEnum = {};
		$scope.modalInfo.path = "";
		$scope.modalInfo.stateEnum.positionDataSyncHistoryDetail = 1;           //详情
	...
	$scope.closeSyncHistoryData = function () {
		mainService.hideModal("positionDataSyncHistory_modal_back");
	...
	//产看详情
	$scope.clickCheckPositionDetail = function (id, time) {
			//查询对应的详情
			let requestData = {
				body:{
					sno:id,
			...
			positionDataSyncHIstoryService.getTsoperLogDetail(requestData,function(res){
				console.log(res);
				...
				if(res.body.data.length > 0) {
					//有数据
					$scope.positionDataSyncDetail = res.body.data;
				$scope.fundsDataSynchronizeShowModal($scope.modalInfo.stateEnum.positionDataSyncHistoryDetail);
				} else {
					//没有数据
	...
	$scope.getOperatorLogDataInfo = function () {
		...
		var requestData = { body: { begin_date: beginDate, end_date: endDate, page: $scope.currentPage, pageSize: 32, oper_type: "同步持仓数据" } };
		console.log("仓位操作日志");
		console.log(requestData);
		positionDataSyncHIstoryService.getOperatorLogData(requestData, function (retData) {
			...
			if (retData.msret.msgcode == '00') {
				if (retData.body.data.length > 0) {
					...
					cms.message.error("成功提示, 暂时没有同步数据");
				}
	...
	$scope.fundsDataSynchronizeShowModal = function (state) {
		...
		switch (state) {
			case $scope.modalInfo.stateEnum.positionDataSyncHistoryDetail:
				$scope.modalInfo.path = "../positionDataSynchronize/positionDataSyncHistory/positionDataHistoryDetail.html";
				break;
			default:
				break;
		}
	...
	//一级弹框加载完成
	$scope.positionDataSyncHistoryLoadModalReady = function () {
		switch ($scope.modalInfo.state) {
			case $scope.modalInfo.stateEnum.positionDataSyncHistoryDetail:
				mainService.showModal("positionDataSyncHistory_modal_back", "positionDataSyncHistoryDetail_modal", "positionDataSyncHistoryDetail_modal_title");
				break;
			default:
				break;
		}
	...
	$scope.getAllTaMgr = function () {
		...
		positionDataSyncHIstoryService.getAllTaMgr(requestData, function (res) {
			if (res.msret.msgcode != '00') {
			...
			console.log($scope.allAssetManager);
			$scope.$apply();
